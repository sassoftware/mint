...
class TargetConfig(rAAWebPlugin):
    """
    Represents the web side of the plugin.
    """
    # Name to be displayed in the side bar.
    displayName = _("Target Configuration")

    # Name to be displayed on mouse over in the side bar.
    tooltip = _("Plugin for configuring targets")

    @raa.web.expose(template="rPath.targetConfig.index")
    def index(self):
        return dict()
