...
class SurveyManager(basemanager.BaseManager):
    ...
    def _subel(cls, obj, *path):
        return survey_models.modellib.Etree.findChildren(obj, *path)

    @classmethod
    def _findSimple(cls, obj, child, default=None):
    ...
    def addSurveyForSystemFromXml(self, system_id, xml):
        ...
        '''
        xmodel = etree.fromstring(xml)
        return self.addSurveyForSystemFromEtree(system_id, xmodel)
    ...
    def updateSurveyFromXml(self, survey_uuid, xml):
        ...
        for xtag in xtags:
            tag = survey_models.SurveyTag(survey = survey, name = xtag.text)
            tag.save()

        survey.name = self._findSimple(xmodel, 'name')
        survey.description = self._findSimple(xmodel, 'description')
        survey.comment = self._findSimple(xmodel, 'comment')
        survey.removable = self._bool(self._findSimple(xmodel, 'removable', True))
    ...
    def _toxml(self, what, tag_override=None):
        ...
        if what is None:
            ...
            # also, they don't speak English in What.
            return ''
        if tag_override is not None:
            what.tag = tag_override
        return etree.tostring(what)

    def xwalk(self, xvalues, position='', results=None):
        ''' find the leaf nodes in an xobj config model. '''

        # we start walking from xvalues, which is either a list-xobj or
    ...
    def _store_rpm_packages(self, survey, xrpm_packages, rpms_by_info_id, rpm_info_by_id):
        ...
        for xmodel in xrpm_packages:
            xinfo = xmodel.find('rpm_package_info')
            if xinfo is None:
                continue
            # be tolerant of the way epoch comes back from client XML
            epoch = self._i(self._findSimple(xinfo, 'epoch', None))
            info, created = survey_models.RpmPackageInfo.objects.get_or_create(
               name = self._findSimple(xinfo, 'name'),
               version = self._findSimple(xinfo, 'version'),
               epoch = epoch,
               release = self._findSimple(xinfo, 'release'),
               architecture = self._findSimple(xinfo, 'architecture'),
               description = self._findSimple(xinfo, 'description'),
               signature = self._findSimple(xinfo, 'signature'),
            )

            # the ID's coming back from the client are obviously not database
            # IDs and we are storing them here to later associate references in
            # the client XML to associate rpm packages
            # with the conary packages that encapsulate them.
 
            rpm_info_by_id[xmodel.attrib['id']] = info
            pkg = survey_models.SurveyRpmPackage(survey=survey, rpm_package_info = info)
            rpms_by_info_id[info.pk] = pkg
            pkg.install_date = self._date(self._findSimple(xmodel, 'install_date'))
            pkg.save()
    ...
    def _store_conary_packages(self, survey, xconary_packages, topLevelItems, rpm_info_by_id, rpms_by_info_id):
        ...
        for xmodel in xconary_packages:
            xinfo = xmodel.find('conary_package_info')

            # XML from the client comes back frozen every time, but
            # there are tables that have only the unfrozen versions, so
            # we compute that here so we can do lookups into those
            # tables.  This was done for inventory_trove, which may or
            # may not exist in Harpoon, but still useful to store for
            # search purposes.

            unfrozen = ''
            try:
                conary_version = versions.ThawVersion(
                        self._findSimple(xinfo, 'version'))
            ...
            info, created = survey_models.ConaryPackageInfo.objects.get_or_create(
                name = self._findSimple(xinfo, 'name'),
                version = self._findSimple(xinfo, 'version'),
                flavor = self._findSimple(xinfo, 'flavor', ''),
                description = self._findSimple(xinfo, 'description', ''),
                revision = self._findSimple(xinfo, 'revision'),
                architecture = self._findSimple(xinfo, 'architecture', ''),
                signature = self._findSimple(xinfo, 'signature'),
            )
            ...
            encap = xinfo.find('rpm_package_info')
            use_date = self._date(self._findSimple(xmodel, 'install_date'))
            top_level = self._findSimple(xmodel, 'is_top_level') or ''

            # client should send back flags of what's top level or not,
            # but in case it doesn't, group-foo-appliance is also used
            # to mark something top level.  We'll probably have to
            # revisit this when we have to do more with system model.
            ...
            if encap is not None:
                info.rpm_package_info = rpm_info_by_id[encap.attrib['id']]
                info.save()
                # conary may not support install_date yet so cheat and get it from the RPM if available
                if self._findSimple(xmodel, 'install_date') in [ '0', '', None ]:
    ...
    def _save_windows_packages(self, survey, xwindows_packages, windows_packages_by_id):
        ...
        for xmodel in xwindows_packages:
            ...
            xinfo = xmodel.find('windows_package_info')
            if xinfo is None:
                continue
            xid = xmodel.attrib['id']
            info, created = survey_models.WindowsPackageInfo.objects.get_or_create(
                publisher = self._findSimple(xinfo, 'publisher'),
                product_code = self._findSimple(xinfo, 'product_code'),
                product_name = self._findSimple(xinfo, 'product_name'),
                package_code = self._findSimple(xinfo, 'package_code'),
                type = self._findSimple(xinfo, 'type'),
                upgrade_code = self._findSimple(xinfo, 'upgrade_code'),
                version      = self._findSimple(xinfo, 'version')
            )
            pkg = survey_models.SurveyWindowsPackage(
                windows_package_info = info,
                survey = survey,
                install_source = self._findSimple(xmodel, 'install_source'),
                local_package = self._findSimple(xmodel, 'local_package'),
                install_date   = self._date(self._findSimple(xmodel, 'install_date')),
            )
    ...
    def _save_windows_patches(self, survey, xwindows_patches, windows_packages_by_id):
        ...
        for xmodel in xwindows_patches:
            ...
            xinfo = xmodel.find('windows_patch_info')
            if xinfo is None:
                continue
            info,created = survey_models.WindowsPatchInfo.objects.get_or_create(
                display_name = self._findSimple(xinfo, 'display_name'),
                uninstallable = self._bool(self._findSimple(xinfo, 'uninstallable')),
                patch_code = self._findSimple(xinfo, 'patch_code'),
                product_code = self._findSimple(xinfo, 'product_code'),
                transforms = self._findSimple(xinfo, 'transforms'),
            )
            ...
            if created:
                ...
                for rp in referenced_packages_hack:
                    pkg = windows_packages_by_id[rp.attrib['id']]
                ...
                for rp in referenced_packages:
                    package_infos = survey_models.WindowsPackageInfo.objects.filter(
                        publisher = self._findSimple(rp, 'publisher'),
                        product_code = self._findSimple(rp, 'product_code'),
                        package_code = self._findSimple(rp, 'package_code'),
                        type = self._findSimple(rp, 'type'),
                        upgrade_code = self._findSimple(rp, 'upgrade_code'),
                        version = self._findSimple(rp, 'version')
                    )
            ...
            pkg = survey_models.SurveyWindowsPatch(
                ...
                local_package = self._findSimple(xmodel, 'local_package'),
                install_date = self._date(self._findSimple(xmodel, 'install_date')),
                is_installed = self._bool(self._findSimple(xmodel, 'is_installed'))
            )
            pkg.save()
    ...
    def _save_services(self, survey, xservices):
        ...
        for xmodel in xservices:
            xinfo = xmodel.find('service_info')
            if xinfo is None:
                continue
            info, created = survey_models.ServiceInfo.objects.get_or_create(
                name = self._findSimple(xinfo, 'name'),
                autostart = self._findSimple(xinfo, 'autostart'),
                runlevels = self._findSimple(xinfo, 'runlevels'),
            )
            service = survey_models.SurveyService(
                service_info = info,
                survey = survey,
                running = self._bool(self._findSimple(xmodel, 'running')),
                status = self._findSimple(xmodel, 'status'),
            )
            service.save()
    ...
    def _save_windows_services(self, survey, xwindows_services):
        ...
        for xmodel in xwindows_services:
            ...
            service = survey_models.SurveyWindowsService(
                ...
                survey = survey,
                status = self._findSimple(xmodel, 'status'),
                running = self._bool(self._findSimple(xmodel, 'running', 'false')),
                start_account = self._findSimple(xmodel, 'start_account', ''),
                start_mode = self._findSimple(xmodel, 'start_mode', ''),
                autostart = autostart
            )
            service.save()
    ...
    def _save_tags(self, survey, xtags):
        ...
        for xmodel in xtags:
            tag = survey_models.SurveyTag(survey = survey,
                    name = self._findSimple(xmodel, 'name'))
            tag.save()

    @exposed
    def addSurveyForSystemFromEtree(self, system_id, model):
        ''' 
        given an XML (etree) model for a survey as POSTed to inventory/systems/N/surveys from a Linux or Windows client, process
        ...
        # if we recieved a top level <values> this is a sign this is a
        # pre-Goad survey, which we no longer support
        if xsurvey.find('values'):
            raise Exception("version 7.0 or later style surveys are required")
        surveyUuid = xsurvey.find('uuid')
        if surveyUuid is None:
            surveyUuid = str(uuid.uuid4())
        else:
            surveyUuid = surveyUuid.text

        # config-properites replaces <values> in Goad-and-later version surveys
        xconfig_properties = xsurvey.find('config_properties')
        ...
        values = xconfig_properties.find('values')
        if values is not None:
             values.tag = 'configuration'
        ...
        if system.configuration is not None:
            ...
            xdesired_properties = etree.fromstring(config)
        # if we recieved no configuration back, fill in an empty one
        if xdesired_properties is None or xdesired_properties.find('configuration') is None:
            xdesired_properties = etree.Element('configuration')

        # get xobj handles to various other XML elements
        origin = self._findSimple(xsurvey, 'origin', 'scanner')
        xobserved_properties = xsurvey.find('observed_properties')
        xdiscovered_properties = xsurvey.find('discovered_properties')
        xvalidation_report = xsurvey.find('validation_report')
        xpreview = xsurvey.find('preview')
        xconfig_descriptor = xsurvey.find('config_properties_descriptor')
        systemModel = xsurvey.find('system_model')
        ...
        if systemModel is None:
            ...
            systemModelContents = self._findSimple(systemModel, 'contents')
            systemModelModifiedDate = datetime.utcfromtimestamp(int(
                self._findSimple(systemModel, 'modified_date', 0)))
            hasSystemModel = (systemModelContents is not None)

        # get the date, description, and time off the survey XML
        created_date = self._findSimple(xsurvey, 'created_date', 0)
        created_date = datetime.utcfromtimestamp(int(created_date))
        desc    = self._findSimple(xsurvey, 'description', "")
        comment = self._findSimple(xsurvey, 'comment',     "")
        ...
        survey = survey_models.Survey(
            name = system.name, uuid = surveyUuid, description = desc, comment = comment,
