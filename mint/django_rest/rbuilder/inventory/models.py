...
class Inventory(modellib.XObjModel):
    ...
    def __init__(self):
        # FIXME
        # Field definitions need to be moved outside the init to the class level
        # See _serialize_hrefs in basemodels.py for possible cause
        self.zones = D(modellib.XObjHrefModel('zones'), "an entry point into the inventory management zones collection")
        self.management_nodes = D(modellib.XObjHrefModel('management_nodes'), "an entry point into the inventory management nodes collection (rPath Update Services)")
        self.management_interfaces = D(modellib.XObjHrefModel('management_interfaces'), "an entry point into the collection of management interfaces (CIM, WMI, etc.)")
        self.system_types = D(modellib.XObjHrefModel('system_types'), "an entry point into the inventory system types collection")
        self.networks = D(modellib.XObjHrefModel('networks'), "an entry point into the inventory system networks collection")
        self.systems = D(modellib.XObjHrefModel('systems'), "an entry point into the collection of all systems (all systems in inventory_systems and infrastructure systems combined)")
        self.log = D(modellib.XObjHrefModel('log'), "an entry point into inventory logging")
        self.event_types = D(modellib.XObjHrefModel('event_types'), "an entry point into the inventory events collection")
        self.system_states = D(modellib.XObjHrefModel('system_states'), "an entry point into the inventory system states collection")
        self.job_states = D(modellib.XObjHrefModel('job_states'), "an entry point into the inventory job states collection")
        self.inventory_systems = D(modellib.XObjHrefModel('inventory_systems'), "an entry point into the collection of inventory systems (all systems visible in the UI under Systems)")
        self.infrastructure_systems = D(modellib.XObjHrefModel('infrastructure_systems'), "an entry point into the collection of infrastructure systems (all systems visible in the UI under Infrastructure)")
        self.image_import_metadata_descriptor = D(modellib.XObjHrefModel('image_import_metadata_descriptor'), 'No documentation')
...
class Networks(modellib.XObjModel):
    ...
    def __init__(self):
        self.systems = D(modellib.XObjHrefModel('../systems'), "an entry point into system inventory")
