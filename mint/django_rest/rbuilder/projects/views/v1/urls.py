...
urlpatterns = patterns('',
    ...
    URL(r'/(?P<project_short_name>(\w|\-)*)/project_branches/(?P<project_branch_label>[a-zA-Z0-9]+(\.|\w|\-|\@|\:)*)/project_branch_stages/(?P<stage_name>(\w|-)+)/images/?$',
        ...
        model='images.Images'),
    URL(r'/(?P<project_short_name>(\w|\-)*)/project_branches/(?P<project_branch_label>[a-zA-Z0-9]+(\.|\w|\-|\@|\:)*)/project_branch_stages/(?P<stage_name>(\w|-)+)/latest_image_file_by_type/(?P<image_type_name>(\w|\-)*)/?$',
        views.ProjectBranchStageLatestImageFileByTypeService(),
        name='ProjectBranchStageLatestImageFileByType'),
