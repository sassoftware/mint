...
class MintServer(object):
    ...
    def getReleaseList(self, limit, offset):
        cu = self.db.cursor()
        r = cu.execute("SELECT Projects.name, Projects.hostname, releaseId FROM Releases LEFT JOIN Projects ON Projects.projectId = Releases.projectId WHERE Projects.hidden=0 AND Projects.disabled=0 ORDER BY timePublished DESC LIMIT ? OFFSET ?", limit, offset)
