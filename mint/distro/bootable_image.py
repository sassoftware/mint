...
class BootableImage(ImageGenerator):
    ...
    def _setupGrub(self):
        ...
        # The following will allow xen domU images to run with pygrub and
        # use the kernel inside the image instead of relying on the dom0
        # to have a kernel installed.
        xen = False
        dom0 = False
        if [x for x in bootDirFiles if re.match('vmlinuz-.*xen.*', x)]:
            xen = True
        if [x for x in bootDirFiles if re.match('xen.gz-.*', x)]:
            dom0 = True
...
title %(name)s (%(kversion)s)
    root (hd0,0)
    kernel /boot/xen.gz-%(kversion)s
    module /boot/vmlinuz-%(kversion)s ro root=LABEL=/
    %(initrdCmd)s

""" % macros % macros) # repeated to cover recursive nature of initrdCmd
        elif self.build.buildType == buildtypes.VMWARE_IMAGE: 
            arch =  self.build.getArch()
            if arch == 'x86':
                macros.update({'clock' : 'clock=pit'})
            elif arch == 'x86_64':
                macros.update({'clock' : 'notsc'})
            else:
                macros.update({clock : '' })
            fd.write("""
#grub.conf generated by rBuilder
...
title %(name)s (%(kversion)s)
    root (hd0,0)
    kernel /boot/vmlinuz-%(kversion)s ro root=LABEL=/ %(clock)s
    %(initrdCmd)s
...
title %(name)s (%(kversion)s)
    root (hd0,0)
    kernel /boot/vmlinuz-%(kversion)s ro root=LABEL=/
    %(initrdCmd)s
...
""" % macros % macros) # repeated to cover recursive nature of initrdCmd
    ...
    def fileSystemOddsNEnds(self):
        ...
        copyfile(os.path.join(self.imgcfg.dataDir, 'hosts'), os.path.join(self.fakeroot, 'etc', 'hosts'))
        copyfile(os.path.join(self.imgcfg.dataDir, 'network'), os.path.join(self.fakeroot, 'etc', 'sysconfig', 'network'))
        copyfile(os.path.join(self.imgcfg.dataDir, 'ifcfg-eth0'), os.path.join(self.fakeroot, 'etc', 'sysconfig', 'network-scripts', 'ifcfg-eth0'))
        copyfile(os.path.join(self.imgcfg.dataDir, 'keyboard'), os.path.join(self.fakeroot, 'etc', 'sysconfig', 'keyboard'))
        #Was X installed?
        if os.path.isfile(os.path.join(self.fakeroot, 'usr', 'bin', 'X11', 'X')):
            #copy in the xorg.conf file
            copyfile(os.path.join(self.imgcfg.dataDir, 'xorg.conf'), os.path.join(self.fakeroot, 'etc', 'X11', 'xorg.conf'))
        ...
        if os.path.isfile(os.path.join(self.fakeroot, 'usr', 'sbin', 'gpm')):
            copyfile(os.path.join(self.imgcfg.dataDir, 'mouse'), os.path.join(self.fakeroot, 'etc', 'sysconfig', 'mouse'))
    ...
    def runTagScripts(self, target = 'ext3'):
        if not os.path.exists(self.imgcfg.umlKernel[self.arch]):
            raise RuntimeError, "UML kernel not found: %s -- please check configuration of jobserver" % \
                self.imgcfg.umlKernel[self.arch]
