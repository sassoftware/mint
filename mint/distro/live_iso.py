...
class LiveIso(bootable_image.BootableImage):
    ...
    def mkinitrd(self):
        ...
        kMods = ('loop',)
        if self.build.getDataValue('unionfs'):
            kMods += ('unionfs',)
...
"""
    ...
    def makeLiveCdTree(self):
        ...
        f.write(isolinuxCfg % (self.build.getName(), self.getVolName()))
        f.close()

        # tweaks to make read-only filesystem possible.
        if not self.build.getDataValue('unionfs'):
    ...
    def finalizeIso(self):
        ...
        # zip the final build if the main image wasn't compressed already
        if not self.build.getDataValue('zisofs'):
    ...
    def write(self):
        try:
            ...
            self.createImage(self.build.getDataValue('zisofs') and \
                             'zisofs' or 'isofs')
