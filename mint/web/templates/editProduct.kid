...
from mint.producttypes import typeNames
...
<html xmlns="http://www.w3.org/1999/xhtml"
    ...
    <div py:def="breadcrumb()" py:strip="True">
        ...
        <a href="#">${(isNewProduct and "Create New" or "Edit") + " Product"}</a>
    </div>
    <?python
        for var in ['productId', 'trove', 'productName']:
            kwargs[var] = kwargs.get(var, '')
    ?>

    <head>
        <title>${formatTitle((isNewProduct and "Create New" or "Edit") + " Product")}</title>
    </head>
    <?python
        if isNewProduct:
            jsonload = "javascript:getTroveList(" + str(project.getId()) + ");"
        else:
            jsonload = "javascript:handleProductTypes(\""+ arch +"\");"
    ?>
    <body py:attrs="{'onload': jsonload }">
        <div id="layout">
            ...
            <div id="middle">
                ...
                <h2>${isNewProduct and "Create" or "Edit"} Product</h2>

                <form method="post" action="saveProduct" id="mainForm">

                    <div class="formgroupTitle">Distribution Information</div>
                    <div class="formgroup">
                        <label for="relname">Name</label>
                        <input id="relname" name="name" type="text" value="${product.getName()}" /><div class="clearleft">&nbsp;</div>

                        <label for="reldesc">Description (optional)</label>
                        <textarea id="reldesc" name="desc" type="text" py:content="product.getDesc()" /><div class="clearleft">&nbsp;</div>

                    </div>

                    <div class="formgroupTitle">Product Contents<span id="baton"></span></div>
                    <div class="formgroup">
                        <label for="trove">Distribution Group</label>
                        <div py:strip="True" py:if="isNewProduct">
                            <select py:if="isNewProduct" onchange="javascript:onTroveChange(${project.getId()});" id="trove" name="trove">
                                <option value=""></option>
                            </select>
                            <img src="${cfg.staticPath}apps/mint/images/circle-ball-dark-antialiased.gif" id="nameSpinner" style="float: right;"/>
                        </div>
                        <div py:strip="True" py:if="not isNewProduct">
                        ...
                        <div py:strip="True" py:if="not isNewProduct">
                            <input type="text" id="arch" name="archDisplay" value="${not isNewProduct and arch or None}" disabled="disabled" />
                            <input type="hidden" name="arch" value="${not isNewProduct and arch or None}" />
                            <img src="${cfg.staticPath}apps/mint/images/circle-ball-dark-antialiased.gif" id="archSpinner" style="float: right;"/>
                        </div>
                        <div class="clearleft">&nbsp;</div>
                    ...
                    <div class="formgroupTitle">Product Types</div>
                    <div class="formgroup">
                        <div py:strip="True" py:for="key in self.cfg.visibleProductTypes">
                            <input class="reversed" id="producttype_${key}" name="producttype" value="${key}" onclick="javascript:onProductTypeChange('formgroup_${key}');" type="radio" py:attrs="{'checked': (key == productType) and 'checked' or None}" />
                            <label class="reversed" for="producttype_${key}">${typeNames[key]}</label><div class="clearleft">&nbsp;</div>
                        </div>
                    </div>

                    <?python
                        templates = product.getDisplayTemplates()
                        dataDict = product.getDataDict()
                        defaultTemplate = product.productType and product.productType or 1
                    ?>
                    ...
                    <p>
                        <button id="submitButton" type="submit" py:attrs="{'disabled': isNewProduct and 'disabled' or None}">${isNewProduct and "Create" or "Recreate"} Product</button>
                    </p>
